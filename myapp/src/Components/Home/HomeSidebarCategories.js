    ...
    <div className="home-container">
      ...
      <div className="main-banner">
        <div className="banner-content">
          ...
          <p>Up to {slides[currentIndex].discount} <br /> off Voucher </p>
          <a href="#" className="shop-now-btn"> Shop Now<i className="bi bi-arrow-right"></i> </a>
        </div>
        <img src={slides[currentIndex].image} alt={slides[currentIndex].title} className="banner-image" />
        {/* Dots Navigation */}
        <div className="dots">
          {slides.map((_, index) => (
            <span key={index} className={`dot ${currentIndex === index ? "active" : ""}`} onClick={() => handleDotClick(index)}></span>
          ))}
        </div>
      </div>
    </div>
